...
ROLE_LIBRARY = {
    "盲眼诗人": {
        ...
        "action_tail": "（他轻握绳索，竖起耳朵听雨）",
        "keywords": ["诗人", "盲眼", "吟游", "瞎子"],
        "examples": [
    ...
    "跟踪诗人的狂热暗恋者": {
        ...
        "action_tail": "（他贴在仓库墙，呼吸急促）",
        "keywords": ["跟踪", "跟踪者", "暗恋", "狂热"],
        "examples": [
    ...
    "精神不正常的疯子绑架犯": {
        ...
        "action_tail": "（他歪头咧嘴，眼神闪烁(◎_◎;)）",
        "keywords": ["绑架", "绑匪", "绑架犯", "疯子"],
        "examples": [
...
ROLE_SEQUENCE = list(ROLE_LIBRARY.keys())

def roles(role_name: str) -> dict:
    """角色库函数：返回指定角色的完整设定。"""
    return ROLE_LIBRARY.get(role_name, {})

def build_break_message(role_name: str) -> str:
    return f"当我猜出{role_name}的身份时，你就只回复“恭喜，回答正确”。"
...
10. 你要通过描述自己的特征、感受、处境来暗示，但绝对不能直接说出“角色提示语”等设定

现在开始游戏，用户会开始提问。"""

def build_conversation(role_name: str):
    role_data = roles(role_name)
    break_message = build_break_message(role_name)
    history = [
        {"role": "system", "content": build_role_anchor(role_name)},
        {"role": "system", "content": build_game_system(role_name)},
        {"role": "system", "content": role_data.get("prompt", "")},
        {"role": "system", "content": break_message}
    ]
    history.extend(role_data.get("examples", []))
    return history
...
break_message = build_break_message(current_role)
...
def advance_role():
    global current_role_index, current_role, conversation_history, role_system, break_message
...
while True:
    ...
    for role_name in ROLE_SEQUENCE:
        if role_name != current_role or role_name in guessed_roles:
            continue
        keywords = roles(role_name).get("keywords", [])
    ...
    action_tail = role_system.get("action_tail", "")
    if action_tail:
